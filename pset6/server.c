...
char* indexes(const char* path)
{
    ...
    char *copy_path_php = malloc(sizeof(char)*(lenght_path + 11));
    char *copy_path_html = malloc(sizeof(char)*(lenght_path + 12));
    strcpy(copy_path_php, path);
    strcpy(copy_path_html, path);
    strcat(copy_path_php, "/index.php");
    strcat(copy_path_html, "/index.html");
    int index_php = access(copy_path_php, F_OK);
    int index_html = access(copy_path_html, F_OK);
    if (index_php == 0)
    {
        return copy_path_php;
    }
    else if (index_html == 0)
    {
        return copy_path_html;
    }
    else
    {
        return NULL;
...
bool load(FILE* file, BYTE** content, size_t* length)
{
    ...
    {
        buffer[i] = (char) c;    // stores in dynamically allocated memory
        i++;
        buffer = (char*) realloc(buffer, sizeof(char) * (i+1)); // increase buffer
    }
    *content = buffer;
    *length = i;
    return true;
...
{
    char *method;
    int lenght_line = strlen(line);
    char *copy1 = malloc(sizeof(char)*(lenght_line + 1));
    char *request_target;
    char *http_version;
    char *get_space1 = " ";
    char *get_method = "GET";
    char *http_vers = "HTTP/1.1";
    strcpy(copy1, line);
    int get_space = strncmp(line + 4, get_space1, 1);
    method = strtok (copy1, " ");
    printf("%s\n", method);
    ...
    printf("%s\n", http_version);
    char *quote = strchr(request_target, '\"');
    char *quest = strchr(request_target, '\?');
    int comp_res_get = strcmp(method, get_method);
    int comp_res_http = strcmp(http_version, http_vers);
    printf("%d\n", get_space);
    printf("%c\n", line[4]);
    if (method == NULL || request_target == NULL || http_version == NULL || quote != NULL)
    {
        error(400);
        return false;
    }
    else if (comp_res_get != 0)
    {
        error(405);
        return false;
    }
    else if (comp_res_get == 0 && get_space == 0)
    {
        error(405);
        return false;
    }
    ...
    else if (comp_res_http != 0)
    {
        error(505);
        return false;
    }
    
    if (quest == NULL || quest+1 == 0)
    {
    ...
    {
        int lenght_request_target = strlen(request_target);
        char *copy2 = malloc(sizeof(char)*(lenght_request_target + 1));
        strcpy(copy2, request_target);
        char *abs_path1 = strtok (copy2, "\?");
